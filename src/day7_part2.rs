...
impl Hand {
    fn kind(&self) -> HandKind {
        ...
        let mut m = matches
            .iter()
            .filter(|(_count, card)| **card != Card::Joker);
        let count = m.next().map(|(count, _)| *count).unwrap_or_default();
        let second = m.next();
        let joker_count = matches
            .iter()
            .find(|(_count, card)| **card == Card::Joker)
            .map(|(count, _)| *count)
            .unwrap_or_default();
        let count = joker_count + count;
        match count {
...
mod test {
    ...
    fn part_1() {
        ...
        assert_eq!(
            super::part_2(sample_data.lines().map(|l| l.to_string())),
            "5905".to_string()
        );
    }
    ...
    fn reddit_sample() {
        ...
        let mut hands = sample
            ...
            .collect::<Vec<_>>();
        hands.sort();
        assert_eq!(super::part_2(sample.lines().map(|l| l.to_string())), "6839");
    }
}
